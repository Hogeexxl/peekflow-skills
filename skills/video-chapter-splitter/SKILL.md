...
description: 根据字幕语义自动识别章节边界，并将长视频切成独立片段。
---
...
16kHz mono WAV (~180MB)
  ↓ Whisper 本地转录 (自动侦测语言, M1/M1Pro medium模型)
字幕文本（带精确时间戳）
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--model` | medium | Whisper 模型 (CPU 推荐 small, Mac M1/M1Pro 推荐 medium) |
| `--extract-only` | - | 只提取字幕 |
...
| tiny | 最快 | 一般 | 快速预览 |
| base | 快 | 较好 | 英文内容 |
| small | 中等 | 好 | 普通 CPU 首选 |
| **medium** | **可接受** | **很好** | **M1/M1Pro 及 GPU 推荐 (当前默认)** |
| large | 最慢 | 最好 | 高精度需求 |
...
### Whisper 识别慢
尝试加上 `--model small` 参数退回小模型 + 先提取音频（v2.0 默认行为）
